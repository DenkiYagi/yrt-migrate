...
import { detectXmlType, migrateTo2026_2 } from "./migrate_2026_2.mjs";
...
function getDefaultOutputDir(inputPath, isDirectory) {
    if (isDirectory) {
        ...
        const baseName = dirName.replace(/-(?:v1\.0|2025\.1|2026\.1)$/u, "");
        return path.join(parentDir, `${baseName}-2026.2`);
    }

    const parsed = path.parse(inputPath);
    if (parsed.dir === "" || parsed.dir === ".") {
        return path.join(".", `${parsed.name}-2026.2`);
    }
 ...
 */
export async function migrateFrom2026_1(args, inputPath, outputDir) {
    ...
    for (const file of inputFiles) {
        const migrated = migrateTo2026_2(file.xml, file.type);
