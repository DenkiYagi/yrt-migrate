...
出力はマイグレーション済みの `layout-*.xml` と、必要に応じて単一の `style.xml` です。既存の `layout-*.xml` と `style.xml` が出力先にある場合は、新しい結果を書き出す前に削除されます。
...
下記コマンドで実行します。実行後はマイグレーション済みの `layout-*.xml`（および必要に応じて `style.xml`）が出力先ディレクトリに保存されます。既に同名ファイルが存在する場合は先に削除されるため、古い出力が残りません。出力先を指定しない場合、入力ファイルと同じディレクトリに `<入力ファイル名>-v1.0` が自動生成されます。
