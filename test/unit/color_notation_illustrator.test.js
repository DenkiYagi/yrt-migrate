...
describe("カラー記法のIllustrator寄り変換マイグレーション", () => {
    ...
    describe("4方向系カラー属性のスペース区切り複数値", () => {
        it("4方向系カラー属性がスペース区切りで複数値の場合、すべて変換される", () => {
            ...
            expect(migrated.layouts[0].xml).toContain('borderColor="K100 K50 K0"');
            expect(migrated.layouts[0].xml).toContain('outerBorderColor="R100G0B0 R0G100B0 R0G0B100"');
        });
        ...
        it("4方向系カラー属性がスペース複数区切りでもすべて変換される", () => {
            ...
            const yrtDocument = { layouts: [{ name: null, xml: inputXml }], style: null, assets: null };
            const migrated = migrate(yrtDocument);
            expect(migrated.layouts[0].xml).toContain('borderColor="K100 K0"');
            expect(migrated.layouts[0].xml).toContain('outerBorderColor="R100G0B0 R0G0B100"');
        });
    });
});
