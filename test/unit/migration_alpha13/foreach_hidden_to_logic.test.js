import { migrate } from "../../../src/migration_alpha13/foreach_hidden_to_logic.mjs";
...
describe("foreach/hidden属性→logic属性マイグレーション", () => {
    ...
    it("foreach属性とhidden属性が両方ある場合は変換しない", () => {
        const inputXml = [
            '<?xml version="1.0" encoding="UTF-8"?>',
            '<Grid foreach="${items}" hidden="isHidden"/>'].join('\n');
        const yrtDocument = { layouts: [inputXml], style: null };
        const { warnings, result: migrated } = withWarningSpy(diagnostics => migrate(yrtDocument, inputXml));
        const xml = migrated.layouts[0];
        expect(xml).toContain('foreach="${items}"');
